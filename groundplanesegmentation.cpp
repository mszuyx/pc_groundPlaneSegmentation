...
GroundPlaneSeg::GroundPlaneSeg():node_handle_("~"){
    ...
    node_handle_.param("sensor_height", sensor_height_, 1.0);  //1.0
    ROS_INFO("Sensor Height: %f", sensor_height_);

    node_handle_.param("num_iter", num_iter_, 5); //5
    ROS_INFO("Num of Iteration: %d", num_iter_);

    node_handle_.param("num_lpr", num_lpr_, 200); //200
    ROS_INFO("Num of LPR: %d", num_lpr_);

    node_handle_.param("th_seeds", th_seeds_, 1.2); //1.2
    ROS_INFO("Seeds Threshold: %f", th_seeds_);

    node_handle_.param("th_dist", th_dist_, 0.1); //0.1
    ROS_INFO("Distance Threshold: %f", th_dist_);
    
    node_handle_.param("th_ceil_", th_ceil_, 1.0); //1.0
    ROS_INFO("Ceiling Threshold: %f", th_ceil_);
    
    node_handle_.param("th_box_", th_box_, 3.0); //3.0
...
void GroundPlaneSeg::rs_pc_callback_ (const sensor_msgs::PointCloud2ConstPtr& input_cloud, const sensor_msgs::Imu::ConstPtr& imu_msg){
     ...
     //std::cout << imu_msg->orientation << std::endl;
     ...
     */
     ...
     for(int i=0;i<num_iter_;i++){
         ...
         not_ground_pc->clear();
         for(size_t r=0;r<cloud_org->points.size();r++){
             if(double((*cloud_org)[r].y)>th_dist_d_ && double((*cloud_org)[r].y)<(th_dist_d_+(2*th_dist_))){
                 ground_pc->points.push_back((*cloud_org)[r]);
             }else if(double((*cloud_org)[r].y)<-th_ceil_){ //th_ceil_
                 continue;
             }else{
                 not_ground_pc->points.push_back((*cloud_org)[r]);
             }
          }
         }
     }
